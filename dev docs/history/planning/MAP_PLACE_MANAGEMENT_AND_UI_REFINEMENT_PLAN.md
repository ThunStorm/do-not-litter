...
> 已归档（2026-09-08）：本计划所列 Work Package 已由 `161d320` 覆盖；原计划的描述是实施前基线，不替代当前源码状态。现状见 [实施状态](../../IMPLEMENTATION_STATUS.md)，推荐评分和自动行程见 [Future Roadmap](../../planning/FUTURE_ROADMAP.md)。

> 原建议仓库落点：`dev docs/planning/MAP_PLACE_MANAGEMENT_AND_UI_REFINEMENT_PLAN.md`（现归档于本目录）
