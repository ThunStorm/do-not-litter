> 历史工程计划，2026-09-03 归档；不是当前执行指令或待办。项目背景见 [根 README](../../README.md)，当前范围见 [实施状态](../IMPLEMENTATION_STATUS.md)。裸文档路径以 dev docs/ 为基准。
...
1. 【旧读取规则已撤销】遵循仓库 AGENTS.md 和 CODEX_CONTEXT.md，只按当前任务读取相关章节。
...
真实样本、Fixture 规则和初始质量门槛见 `testing/GOLDEN_SAMPLES.md`。
...
- 性能门槛使用 Mac mini 实测值回填 `testing/GOLDEN_SAMPLES.md`。
...
> Phase 15–18 的实施必须同时遵循 `video/VIDEO_AI_NOTE_PIPELINE.md` 与 `video/VIDEO_AI_NOTE_IMPLEMENTATION_GUIDE.md`，按指南中的 Work Package 拆分提交，不得一次性重写 Pipeline 或提前实现未确认 UI。
...
## Acceptance
测试视频 Fixture 或真实样本可得到 Transcript；有字幕时不下载音频，无字幕时自动 ASR；分 P 的字幕、元数据和时间跳转属于同一集。详细标准见 `video/VIDEO_AI_NOTE_PIPELINE.md`。
...
## Acceptance
完整执行 `video/VIDEO_NOTE_READING_EXPERIENCE_V042_SPEC.md` 第 10 节，不得以独立截图宫格或原始转录堆叠代替。
...
## Acceptance
完整执行 `video/VIDEO_NOTE_LIST_V043_SPEC.md` 第 9 节；封面失败不得阻塞 Note，不得长期热链远程 CDN。
...
## Acceptance
完整执行 `video/VIDEO_NOTE_DELETE_V044_SPEC.md`，列表和详情入口使用同一 Service。
...
ERROR 步骤在 Artifact 有效期内只重跑当前及下游；上游不产生新外部调用。完整执行 `jobs/PIPELINE_STEP_REPLAY_V044_SPEC.md`。
