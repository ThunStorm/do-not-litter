...
function VideoPlaceCandidate({ place, review, canonicalUrl, jump, jumpToReview }: { place: VideoPlace; review?: PlaceReview; canonicalUrl: string; jump: (sectionId?: string | null) => void; jumpToReview: (mentionId: string) => void }) {
  ...
  return <div className="video-place"><button onClick={() => jump(place.target_section_id)}><strong>{place.name}</strong><small>待确认</small></button>{place.start_ms != null ? <a className="video-place__time-link" href={videoTimestampUrl(canonicalUrl, place.start_ms)} target="_blank" rel="noreferrer" aria-label={`在新窗口打开视频并跳至 ${timecode(place.start_ms)}`} title="在新窗口打开视频并跳至此时间"><time>{timecode(place.start_ms)}</time></a> : null}<InsightChips insights={place.insights} jump={jump} />{review ? <button className="text-action" onClick={() => jumpToReview(place.id)}>确认 POI</button> : null}</div>
}
