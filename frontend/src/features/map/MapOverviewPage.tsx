...
export function MapOverviewPage() {
  ...
  const overview = useQuery({ queryKey: ['map', selectedId, state, deferredQuery, placeType, season, month, monthSegment, dayTimeSlot, visibility, routeId, viewport], queryFn: () => api.map({ selectedPlaceId: selectedId, state, query: deferredQuery, placeType, season, month, monthSegment, dayTimeSlot, visibility, routeId, bbox: viewport.bbox, zoom: viewport.zoom }) })
